...
- （11F）搜索框按联系方式**精确**搜卖家（`lib/search/contactSearch.ts`，在 `GET /api/search?site=items` 里）：只认**整串相等**——9A 去掉的是子串匹配（能逐字试出别人的联系方式），整串相等与既有的 `by-contact` 是同一个能力。命中后才过 `gateReveal`（tag `by:<值>`，与 by-contact 同 tag 只计一次），因此也进 11D 的异常侦测；配额拒绝 / bot → 退回普通关键词结果，与「没有这个卖家」无法区分。响应带 `sellerMatch.anchorId`、不回显联系方式；前端命中时**不把搜索词写进地址栏**。
