...
- （11D）异常侦测 `lib/abuseWatch.ts`：`gateReveal` 每次（放行与被拒都算）fire-and-forget 过一遍——同一访客 1h 内不同披露目标数 ≥ 30、或同一 IP 1h 内不同访客身份数 ≥ 8 → **立即发邮件**（`ALERT_EMAIL_TO`，退到 `DIGEST_EMAIL_TO`；用配额表 `alert:<kind>:<subject>` 每小时 1 次去重；邮件不含 IP 与完整 visitorId）。24h 内不同目标数 ≥ 10 的访客 → `/admin`「异常访问」小节 + 摘要 `revealWideVisitors24h`（阈值 `wideReveal`）。**只通知，不自动处置**。阈值 env 可调。
