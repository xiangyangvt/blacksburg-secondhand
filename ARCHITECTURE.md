...
| 搜索（10A 起） | `lib/search/embedText.ts`（白名单取字段 → 文本，纯函数）· `vectorStore.ts`（`pgvector` raw SQL / `json` SQLite 两实现，按 `DATABASE_URL` 选；HNSW 索引运行时幂等建）· `indexer.ts`（写入时异步 embed / remove，`substantiveChanged` 判实质性变更）· `backfill.ts` + `scripts/backfill-embeddings.ts` + `api/admin/backfill-embeddings` · **10B**：`lib/itemsQuery.ts`（列表 where / orderBy / 序列化，`api/items` 与 `api/search` 共用）· `lib/search/hybrid.ts`（总开关 `SEARCH_AI_ENABLED`、查询词 embedding 10 分钟缓存、`pickSemantic` 去重与阈值、语义路配额 60 次 / 小时）· `api/search`（第 0 层 + 第 1 层；**10B-2** 起三站共用：`site=items|listings|events`，每站一个 handler 提供"关键词层 / 候选 id / 按 id 取公开卡片"，语义逻辑共用）· `lib/listingsQuery.ts` `lib/eventsQuery.ts`（两个列表接口的解析 / where / 序列化，与 `api/search` 共用）· `lib/useSemanticLayer.ts`（室友 / 活动页的客户端 hook：这两页的关键词搜索是本地过滤，用 `kw=<本地命中数>` 告诉服务端）· 组件 `SemanticResults`（不绑定卡片类型，`renderCard` 由页面传；`ItemCard` / `ListingCard` / `EventCard` 加 `badge`）· **10C**：`lib/search/chat.ts`（配额门、历史清洗、白名单候选行与 prompt、输出校验）· `lib/llmUsage.ts`（计费流水、当日预算熔断；**10D**：`recordRejection` 记 429 / 503 事件行——零费用、进程内每分钟最多 60 行防写放大，`usageSummary` 按天 / 按月汇总）· `components/admin/AiCostSection.tsx`（`/admin` 的 AI 费用小节，独立服务端组件）· 摘要 `digest.ts` 带 `aiCostUsd` / `aiBudgetTripped`，阈值 `aiCost`· `api/search/chat`（SSE）· 组件 `SearchChat`（就地展开的对话区，状态只在内存）· **11E 统一问询栏**：`SearchBox` 传 `ask` 时（v1 只有二手站）一个栏同时是搜索 / 问 AI / 问站长——打字走关键词，回车或点发送把那句话交给 `SearchChat`（不再有自己的输入框，列表上方就地展开）；对话输出多一个 `intent`，`ask_ops`（问站务 / 反馈）时 **LLM 只分类不回答**，服务端换固定文案并发 `event: intent`，前端出 `FeedbackCard`（预填原话，用户点发送才提交）。不依赖 AI 的兜底：零结果 / 对话报错 / 触发配额时一行 `FeedbackLink`。`lib/feedback.ts` + `api/feedback` + `Feedback` 表 + `/admin`「用户反馈」小节 + 摘要 `feedbackOpen`（阈值 `feedback`）· **11A 卖家摊位**：`Shelf` 表（`slug` ↔ `contactValue`，同一卖家永远同一 slug）· `lib/shelf.ts` · `POST /api/shelf`（「联系方式 + 编辑码」验身份后取或建）· `/s/<slug>`（长图二维码落点，跳 `/?shelf=<slug>&utm_source=wx_poster`）· `itemsQuery.resolveSeller`（`shelf` 优先，其次 9A 的 `sameSellerAs`；列表 / 搜索 / 对话三个接口共用）· **11B**：`VectorStore.nearestToRows`（以库里已有行的向量为引子，零费用）· `lib/search/similar.ts` · `GET /api/items/similar`（向量不可用兜底同类目）· 组件 `SellerExplore`（卖家列表后接「相似的」+「全站其他在售」）· **11C 一键长图**：`lib/poster.ts`（取数与版式，纯函数：按显示宽度截断、分页每张 ≤ 10 件、字形集合）· `GET /api/poster/<slug>?page=N`（nodejs runtime，`@vercel/og` 出 1080 宽 PNG；内容原样取自帖子、不经 AI、不含联系方式；底部二维码 → `/s/<slug>`，依赖 `qrcode`；中文字体按本图字形向 Google Fonts 要 TTF 子集，进程内缓存，拿不到 → 503）· 组件 `PosterModal` + 「我的 → 在售」里的按钮（先 `POST /api/shelf` 再出图；图是真实 `<img>` 地址，微信里长按保存） | prisma · llm · rateLimit | items / listings / events 的 POST / PATCH / DELETE / publish、reports 自动隐藏、scraper runner；`app/page.tsx` 有关键词时改调 `api/search` |
...
- （11E）反馈 `POST /api/feedback`：bot UA 403；同 visitor 5 条 / 小时、15 条 / 天，同 IP 20 条 / 小时（`feedback:vid:<id>:h|d` `feedback:ip:<ip>:h`）。`Feedback.contact` 是用户自愿留的回访方式：只在 `/admin` 显示，永不下发、永不进 LLM / embedding；`page` 只存路径不存 query。
- （11A）`POST /api/shelf`：bot UA 403；同 visitor / 同 IP 各 20 次 / 小时（每次跑 bcrypt，也防编辑码爆破）。`Shelf.contactValue` 只在服务端用，永不下发、不进 URL；摊位只在卖家本人生成长图时创建（没生成过的卖家不会被 slug 归集）。`/s/<slug>` 与摊位视图 `robots: noindex`。
- （11C）`GET /api/poster/<slug>`：bot UA 403；同 IP 60 次 / 小时（渲染吃 CPU）。**服务端取图只认 `res.cloudinary.com`、不跟随重定向**（`photoUrls` 是用户提交的字符串，入库未限域名；任意 URL 都取即 SSRF），其余当无图。
- （11D）异常侦测 `lib/abuseWatch.ts`：`gateReveal` 每次（放行与被拒都算）fire-and-forget 过一遍——同一访客 1h 内不同披露目标数 ≥ 30、或同一 IP 1h 内不同访客身份数 ≥ 8 → **立即发邮件**（`ALERT_EMAIL_TO`，退到 `DIGEST_EMAIL_TO`；用配额表 `alert:<kind>:<subject>` 每小时 1 次去重；邮件不含 IP 与完整 visitorId）。24h 内不同目标数 ≥ 10 的访客 → `/admin`「异常访问」小节 + 摘要 `revealWideVisitors24h`（阈值 `wideReveal`）。**自动处置**（Sean 2026-09-18）：「同一访客大范围查看」触发时，先暂停该访客查看联系方式 24 小时（配额表里写一行 `block:vid:<visitorId>`，`gateReveal` 入口直接 429；只停这一个动作，浏览 / 搜索 / 发帖不受影响；`/admin`「异常访问」可一键解除；`ABUSE_AUTO_BLOCK=false` 退回只通知），再发邮件并如实写明已暂停。「同一 IP 多身份」**只通知不处置**——校园 NAT 下封 IP 会误伤一片，且轮换 cookie 的人已被 IP 小时配额框住。阈值 env 可调。
- （11F）搜索框按联系方式**精确**搜卖家（`lib/search/contactSearch.ts`，在 `GET /api/search?site=items` 里）：只认**整串相等（忽略大小写**：`lower(col) = lower(CAST(q AS TEXT))` 的参数化原生 SQL，SQLite / Postgres 通用；同一位卖家的几种大小写写法一并带回，配额 tag 归一成小写**）**——9A 去掉的是子串匹配（能逐字试出别人的联系方式），整串相等与既有的 `by-contact` 是同一个能力。命中后才过 `gateReveal`（tag `by:<值>`，与 by-contact 同 tag 只计一次），因此也进 11D 的异常侦测；配额拒绝 / bot → 退回普通关键词结果，与「没有这个卖家」无法区分。响应带 `sellerMatch.anchorId`、不回显联系方式；前端命中时**不把搜索词写进地址栏**。
