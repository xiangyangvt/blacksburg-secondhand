...
| `SEARCH_SEMANTIC_MIN_SIM` | 可选 | 默认 `0.40`（上线实测后从 0.35 上调）。语义候选的余弦相似度阈值 |
...
- （11D）可选 `ALERT_EMAIL_TO`：异常访问「重」级告警的收件邮箱（立即发，同一件事 1 小时一封）；不配则用 `DIGEST_EMAIL_TO`。阈值 `ABUSE_*` 见 `.env.example`。
