# Sprint 10 — 混合搜索与渐进式 AI 助手（已完工）

> 完工于 2026-09-18。五项经 PR #14（10A）、#16（10B，替代被自动关闭的 #15）、#17（10C）、#20（10B-2）与本次 10D 的 PR 合入；另有两个顺带的修复：#18（DeepSeek 默认开 thinking，scraper 调用显式关闭）、#19（`GET /api/listings` 下发留言人 IP 的历史泄露）。
> 2 级风险项经 Codex 跨厂互审：10A 两轮、10B 三轮、10C 四轮、10B-2 一轮，共 40 余条实质发现全部处理，明细在各 PR 评论。2026-09-18 起约定"2 级只跑一轮、1 级不跑"（Codex 额度）。
> **与原 spec 的主要偏差**（细节见各 PR 顶部"偏差"段）：
> - `vectorStore.nearest` 用候选 id 列表而不是 `filterSql`；查询用 `OFFSET 0` 栅栏强制先过滤后精确排序，不走 HNSW 近似（候选小时会漏结果）；HNSW 索引只在回填 / admin 探针路径用 `CONCURRENTLY` 建。
> - 多了 `embedVersion`（乐观版本号，防异步 embed 乱序覆盖）与 `LlmUsage`（10D 的表提前到 10C）。
> - 首页两段式请求（先关键词层，再语义层）；室友站与活动站的关键词搜索本来就是客户端过滤，语义层用 `kw` / `exclude` 与服务端配合。
> - 对话接口不把 LLM 的 token 直接流给用户：服务端完整校验后再以 SSE 分片下发。
> - 预算是"事务 + 按日咨询锁"的先预留后结算，计费 / 配额不可用时 fail closed。
> - **单价**：spec 里的 DeepSeek 价格是过期促销价；按官方页 2026-09-18 的高峰价，每次问答约 0.0024 美元，每天 100 次 ≈ 每月 7 美元（不是 1.5）。每天 2 美元的熔断上限不变。
> - 摘要的 `aiCost` 阈值沿用"0 = 关闭该项"的既有约定（spec 验收写的是"调到 0 触发一封邮件"，改用极小值如 `aiCost=0.000001` 验证）。
> **未能在本机验证、留到上线后做的**：生产 Postgres 上的 pgvector / 咨询锁真实执行；真实 embedding 的跨语言相关性与 0.35 阈值；真实模型的挑选质量与 10 次问答费用汇总。
> 以下为原始执行 spec，原样保留。
