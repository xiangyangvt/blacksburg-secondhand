...
function memDb(): UsageDb & { rows: any[] } {
  ...
  return {
    rows,
    llmUsage: {
      async create({ data }) { const row = { id: `u${++seq}`, ...data }; rows.push(row); return { id: row.id }; },
      async update({ where, data }) { const r = rows.find(x => x.id === where.id); if (!r) throw new Error('not found'); Object.assign(r, data); },
      async delete({ where }) { const i = rows.findIndex(x => x.id === where.id); if (i < 0) throw new Error('not found'); rows.splice(i, 1); },
...
describe('预算:先预留后结算', () => {
  ...
  it('并发临界:余额只够 1 个请求时,20 个并发预留不会全部放行,合计不超预算 + 1 份预留', async () => {
    const db = memDb();
    const budget = 2;
    ...
    // 被拒的预留都撤回了,没有残留
    expect(db.rows.filter(r => String(r.endpoint).endsWith(':reserved'))).toHaveLength(admitted);
  });

  it('预算 0 → 拒绝且不写行;超预算 → 拒绝并撤回', async () => {
    const db = memDb();
  ...
  it('计费表写不了 / 读不了 → unavailable(fail closed),不留残行', async () => {
    const a = memDb(); a.llmUsage.create = async () => { throw new Error('ro'); };
    expect(await reserveBudget(R, a, {} as any, now)).toEqual({ ok: false, reason: 'unavailable' });
    const b = memDb(); b.llmUsage.aggregate = async () => { throw new Error('x'); };
